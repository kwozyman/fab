...
class FabModule:
    ...
    def _read(self):
        ...
        if parsed_source.scheme == "" or parsed_source.scheme == "file":
            try:
                ...
                logging.error(
                    "Could not open file {}: {}".format(parsed_source.path, err)
                )
                raise
            ...
            logging.error(
                'Unknown scheme "{}" in {}'.format(parsed_source.scheme, self.source)
            )
            return False
        return True
    ...
    def _validate(self):
        ...
        if "metadata" not in self.definition:
            logging.warning(
                "No metadata found for module {}. Generating some.".format(self.source)
            )
            ...
            if "name" not in self.definition["metadata"]:
                logging.warning(
                    "No name set in module {}'s metadata. Autosetting to {}".format(
                        self.name, self.name
                    )
                )
            ...
            if "description" not in self.definition["metadata"]:
                logging.warning(
                    "No description set in module {}'s metadata. Autosetting".format(
                        self.name
                    )
                )
                self.definition["metadata"]["description"] = "Autogenerated"

        if "containerfile" not in self.definition:
            logging.debug(
                "'containerfile' missing from definition yaml for module {}, using default".format(
                    self.name
                )
            )
