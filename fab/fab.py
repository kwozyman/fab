...
class Fab:
    ...
    def _validate(self):
        is_valid = True
        if "metadata" not in self.definition:
            logging.warning(
                "No metadata found for fabfile {}. Generating some.".format(self.source)
            )
            self.definition["metadata"] = {}
            self.definition["metadata"]["name"] = self.source
            self.definition["metadata"]["description"] = "Autogenerated"
        else:
            if "name" not in self.definition["metadata"]:
                logging.warning(
                    "No name set in fabfile {} metadata. Autosetting to {}".format(
                        self.name, self.name
                    )
                )
            ...
            if "description" not in self.definition["metadata"]:
                logging.warning(
                    "No description set in fabfile {} metadata. Autosetting".format(
                        self.name
                    )
                )
                self.definition["metadata"]["description"] = "Autogenerated"

        if "from" not in self.definition:
            logging.error('"from" key is required in fabfile')
            is_valid = False

        if "include" not in self.definition:
        ...
        for include in self.definition["include"]:
            if isinstance(include, str):
                ...
                if "buildargs" in include.keys():
                    ...
                    _var_values = {}
            logging.debug(
                "Add new module {} with buildargs {}".format(_include, _var_values)
            )
    ...
    def _run(self, command, args, cwd):
        ...
        process = subprocess.Popen(
            [command] + args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=cwd
        )
        while True:
    ...
    def build(self):
        ...
        for module in self.includes:
            tag = "{}-stage-{}".format(self.name, module.name)
            podman_args = []
